...
- Добавлены 4 новых встроенных Mechvibes-пака:
  ...
  - `EG Crystal Purple`
  - `EG Oreo`
- Исправлен импорт `config-v1 single` (sprite-паки): звук теперь режется по таймкодам `defines`, а не проигрывается целиком.
...
- [x] Anti-overlap микс: `Min gap`, `Release ducking` и окно ducking для снижения наложения `down/up`.
- [x] Тонкая донастройка `release`-огибающих под наушники (`Tail tightness` для хвостов key-up).
- [x] Легкий `pitch variation` (малый диапазон) для более «живого» тембра без артефактов.
- [ ] Финальные тесты latency/CPU и фиксация параметров для стабильного релиза `2.0`.
